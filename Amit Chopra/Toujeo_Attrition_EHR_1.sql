...
drop table if exists ac_pat_rx_basal_nph_glp1;

create table ac_pat_rx_basal_nph_glp1 as
select distinct ptid, rxdate,drug_name,generic_desc,ndc,rx_type,gnrc_nm,a.brnd_nm,
       case when lcase(DRUG_NAME) like '%toujeo%' then 'Toujeo'
              when lcase(GENERIC_DESC) like '%glargine%' and lcase(DRUG_NAME) not like '%toujeo%' then 'Gla-100'
              when lcase(GENERIC_DESC) like '%detemir%' then 'Detemir'
              when lcase(GENERIC_DESC) like '%nph%' then 'NPH'
...
drop table if exists ac_pat_dx_rx;

create table ac_pat_dx_rx as
...
where diag_date between '2014-07-01' and '2021-12-31' group by ptid) a
      ...
      left join (select distinct ptid, min(rxdate) as dt_1st_toujeo from ac_pat_rx_basal_nph_glp1 where rxdate between '2015-01-01' and  '2021-12-31' and rx_type2 in ('Toujeo') group by ptid) c on a.ptid=c.ptid
      left join (select distinct ptid, min(rxdate) as dt_1st_gla_100 from ac_pat_rx_basal_nph_glp1 where rxdate between '2015-01-01' and  '2021-12-31' and rx_type2 in ('Gla-100') group by ptid) d on a.ptid=d.ptid
      left join (select distinct ptid, min(rxdate) as dt_1st_detemir from ac_pat_rx_basal_nph_glp1 where rxdate between '2015-01-01' and  '2021-12-31' and rx_type2 in ('Detemir') group by ptid) e on a.ptid=e.ptid
      left join (select distinct ptid, min(rxdate) as dt_1st_nph from ac_pat_rx_basal_nph_glp1 where rxdate between '2015-01-01' and  '2021-12-31' and rx_type2 in ('NPH') group by ptid) f on a.ptid=f.ptid
group by a.ptid
order by a.ptid
;
...
drop table if exists ac_pat_rx_basal_nph_glp1_index;

create table ac_pat_rx_basal_nph_glp1_index as
select distinct a.*, b.dt_rx_index
from ac_pat_rx_basal_nph_glp1 a left join ac_pat_dx_rx b
on a.ptid=b.ptid
order by a.ptid, a.rxdate
;

select * from ac_pat_rx_basal_nph_glp1_index;
...
drop table if exists ac_lab_a1c_index;

create table ac_lab_a1c_index as
select a.*, b.dt_rx_index
from ac_ehr_t1d_lab_202303 a join ac_pat_dx_rx b
on a.ptid=b.ptid
where isnotnull(b.dt_rx_index) and a.result between 3 and 15
order by a.ptid, a.service_date
;

select * from ac_lab_a1c_index;
...
drop table if exists ac_glp1_a1c_bl_fu;

create table ac_glp1_a1c_bl_fu as
...
from (select ptid, dt_rx_index from ac_pat_dx_rx where isnotnull(dt_rx_index)) a
     left join (select distinct ptid, max(rxdate) as dt_last_glp1_bl from ac_pat_rx_basal_nph_glp1_index where lcase(rx_type) in ('glp1') and rxdate between date_sub(dt_rx_index,540) and date_sub(dt_rx_index,1) group by ptid) b on a.ptid=b.ptid
     left join (select distinct ptid, max(service_date) as dt_last_a1c_bl from ac_lab_a1c_index where service_date between date_sub(dt_rx_index,180) and date_sub(dt_rx_index,1) group by ptid) c on a.ptid=c.ptid
     left join (select distinct ptid, min(service_date) as dt_1st_a1c_fu from ac_lab_a1c_index where service_date between dt_rx_index and dt_rx_index + 180  group by ptid) d on a.ptid=d.ptid
     left join (select distinct ptid, min(rxdate) as dt_1st_glp1_fu from ac_pat_rx_basal_nph_glp1_index where lcase(rx_type) in ('glp1') and rxdate between dt_rx_index and dt_rx_index + 180 group by ptid) e on a.ptid=e.ptid
     left join (select distinct ptid, max(rxdate) as dt_last_insulin_bl from ac_pat_rx_basal_nph_glp1_index where lcase(rx_type) not in ('glp1') and rxdate between date_sub(dt_rx_index,180) and date_sub(dt_rx_index,1) group by ptid) f on a.ptid=f.ptid
group by patid1
order by patid1
;

select * from ac_glp1_a1c_bl_fu;

-- COMMAND ----------

drop table if exists ac_pat_all_enrol;

create table ac_pat_all_enrol as
select distinct a.*, 
...
from ac_pat_dx_rx a left join ac_glp1_a1c_bl_fu b on a.ptid=b.patid1
...
select * from ac_pat_all_enrol;

create or replace table ac_pat_all_enrol_2 as
select distinct * , yr_indx - yrdob as Age_index from ac_pat_all_enrol
order by ptid;

select * from ac_pat_all_enrol_2;

-- COMMAND ----------

drop table if exists ac_patient_attrition_toujeo;

create table ac_patient_attrition_toujeo as
...
union
select ' 2. Have at least one pharmacy fills of Gla-300 or other long-acting BIs during 1/1/2015 and 12/31/2021' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index)
and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
union
select ' 3. Age 18 and above on index date' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index) and age_index>=18 and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
union
select ' 4. At least 6 months continuous medical and pharmacy eligibility prior to index date' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index) and age_index>=18 and first_month_active_new<=dt_rx_index - 180  and dt_rx_index<=last_month_active_new and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
union
select ' 5. At least one weekly GLP-1 RA during the extended baseline period (540 days prior to index date)' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index) and age_index>=18 and first_month_active_new<=dt_rx_index - 180  and dt_rx_index<=last_month_active_new and isnotnull(dt_last_glp1_bl) and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
union
select ' 6. Have at least one valid HbA1c during the baseline period' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index) and age_index>=18 and first_month_active_new<=dt_rx_index - 180  and dt_rx_index<=last_month_active_new and isnotnull(dt_last_glp1_bl) and isnotnull(dt_last_a1c_bl) and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
union
select ' 7. Have at least one valid HbA1c during the follow-up period' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index) and age_index>=18 and first_month_active_new<=dt_rx_index - 180  and dt_rx_index<=last_month_active_new and isnotnull(dt_last_glp1_bl) and isnotnull(dt_last_a1c_bl) and isnotnull(dt_1st_a1c_fu) and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
union
select ' 8. Have at least one same weekly GLP-1 RA during the follow-up period' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index) and age_index>=18 and first_month_active_new<=dt_rx_index - 180  and dt_rx_index<=last_month_active_new and isnotnull(dt_last_glp1_bl) and isnotnull(dt_last_a1c_bl) and isnotnull(dt_1st_a1c_fu) and isnotnull(dt_1st_glp1_fu) and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
union
select ' 9. Those without any T1D diagnoses identified' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index) and age_index>=18 and first_month_active_new<=dt_rx_index - 180  and dt_rx_index<=last_month_active_new and isnotnull(dt_last_glp1_bl) and isnotnull(dt_last_a1c_bl) and isnotnull(dt_1st_a1c_fu) and isnotnull(dt_1st_glp1_fu) and (isnull(dt_1st_t1dm) or (isnotnull(n_t1dm) and n_t2dm>2*n_t1dm)) and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
union
select '10. Those without prior pharmacy fills of insulins (rapid-acting, short-acting, premix, FRC, long-acting, longer-acting BIs) during the baseline period' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index) and age_index>=18 and first_month_active_new<=dt_rx_index - 180  and dt_rx_index<=last_month_active_new and isnotnull(dt_last_glp1_bl) and isnotnull(dt_last_a1c_bl) and isnotnull(dt_1st_a1c_fu) and isnotnull(dt_1st_glp1_fu) and (isnull(dt_1st_t1dm) or (isnotnull(n_t1dm) and n_t2dm>2*n_t1dm)) and isnull(dt_last_insulin_bl) and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
union
select '11. Those without pharmacy fills of more than one basal insulin on the index date' as Step, count(distinct ptid) as n_pat from ac_pat_all_enrol_2 where index_group='Toujeo' and isnotnull(dt_1st_t2dm) and isnotnull(dt_rx_index) and age_index>=18 and first_month_active_new<=dt_rx_index - 180  and dt_rx_index<=last_month_active_new and isnotnull(dt_last_glp1_bl) and isnotnull(dt_last_a1c_bl) and isnotnull(dt_1st_a1c_fu) and isnotnull(dt_1st_glp1_fu) and (isnull(dt_1st_t1dm) or (isnotnull(n_t1dm) and n_t2dm>2*n_t1dm)) and isnull(dt_last_insulin_bl) and not(dt_1st_toujeo=dt_rx_other_insulin and isnotnull(dt_rx_other_insulin) and isnotnull(dt_1st_toujeo)) and dt_1st_t2dm between '2014-07-01' and '2021-12-31' and dt_rx_index between '2015-01-01' and '2021-12-31'
-- union
...
select * from ac_patient_attrition_toujeo;
