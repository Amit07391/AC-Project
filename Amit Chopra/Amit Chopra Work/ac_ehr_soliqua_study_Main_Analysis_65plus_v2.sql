...
create or replace table ac_ehr_sol_study_final_a1c_65plus as
select distinct * from ac_ehr_sol_study_final_a1c_bl_fl
where age_index>=65
order by ptid;

select distinct * from ac_ehr_sol_study_final_a1c_65plus
order by ptid;

-- COMMAND ----------

select mean(bl_hba1c_val) as mn, median(bl_hba1c_val) as med, std(bl_hba1c_val) as std, min(bl_hba1c_val) as min
        , percentile(bl_hba1c_val,.25) as p25, percentile(bl_hba1c_val,.5) as median, percentile(bl_hba1c_val,.75) as p75, max(bl_hba1c_val) as max  from ac_ehr_sol_study_final_a1c_65plus;

        select mean(fl_hba1c_val) as mn, median(fl_hba1c_val) as med, std(fl_hba1c_val) as std, min(fl_hba1c_val) as min
        , percentile(fl_hba1c_val,.25) as p25, percentile(fl_hba1c_val,.5) as median, percentile(fl_hba1c_val,.75) as p75, max(fl_hba1c_val) as max  from ac_ehr_sol_study_final_a1c_65plus;
...
-- COMMAND ----------
 ...
 select mean(diff) as mn, median(diff) as med, std(diff) as std, min(diff) as min
        ...
        from (select distinct ptid,  bl_hba1c_val - fl_hba1c_val as diff from ac_ehr_sol_study_final_a1c_65plus)
...
select count(distinct ptid) from ac_ehr_sol_study_final_a1c_65plus
...
create or replace table ac_ehr_sol_study_final_WT_65plus as
select distinct * from ac_ehr_sol_study_final_WT_bl_fl
where age_index>=65;
select distinct * from ac_ehr_sol_study_final_WT_65plus
order by ptid;

-- COMMAND ----------

select mean(bl_wt_val) as mn, median(bl_wt_val) as med, std(bl_wt_val) as std, min(bl_wt_val) as min
        , percentile(bl_wt_val,.25) as p25, percentile(bl_wt_val,.5) as median, percentile(bl_wt_val,.75) as p75, max(bl_wt_val) as max  from ac_ehr_sol_study_final_WT_65plus
        ...
        , percentile(fl_wt_val,.25) as p25, percentile(fl_wt_val,.5) as median, percentile(fl_wt_val,.75) as p75, max(fl_wt_val) as max  from ac_ehr_sol_study_final_WT_65plus
        where bl_wt_val is not null and fl_wt_val is not null;
...
select mean(diff) as mn, median(diff) as med, std(diff) as std, min(diff) as min
, percentile(diff,.25) as p25, percentile(diff,.5) as median, percentile(diff,.75) as p75, max(diff) as max
from (select distinct ptid,  bl_wt_val - fl_wt_val as diff from ac_ehr_sol_study_final_WT_65plus
where bl_wt_val is not null and fl_wt_val is not null )
...
create or replace table ac_ehr_sol_study_final_BMI_65plus as
select distinct * from ac_ehr_sol_study_final_BMI_bl_fl
where age_index>=65;
select distinct * from ac_ehr_sol_study_final_BMI_65plus
order by ptid;
...
select mean(bl_BMI_val) as mn, median(bl_BMI_val) as med, std(bl_BMI_val) as std, min(bl_BMI_val) as min
        , percentile(bl_BMI_val,.25) as p25, percentile(bl_BMI_val,.5) as median, percentile(bl_BMI_val,.75) as p75, max(bl_BMI_val) as max  from ac_ehr_sol_study_final_BMI_65plus
        ...
        , percentile(fl_BMI_val,.25) as p25, percentile(fl_BMI_val,.5) as median, percentile(fl_BMI_val,.75) as p75, max(fl_BMI_val) as max  from ac_ehr_sol_study_final_BMI_65plus
        where bl_BMI_val is not null and fl_BMI_val is not null;

-- COMMAND ----------

select count(distinct ptid) from ac_ehr_sol_study_final_BMI_65plus
where bl_BMI_val is not null and fl_BMI_val is not null ;
...
-- COMMAND ----------
 ...
 select mean(diff) as mn, median(diff) as med, std(diff) as std, min(diff) as min
        , percentile(diff,.25) as p25, percentile(diff,.5) as median, percentile(diff,.75) as p75, max(diff) as max
        from (select distinct ptid,  bl_BMI_val - fl_BMI_val as diff from ac_ehr_sol_study_final_BMI_65plus
        where bl_BMI_val is not null and fl_BMI_val is not null );
...
select count(distinct ptid) from ac_ehr_soliqua_demo_hba1c_bl_pts
where age_index>=65

-- COMMAND ----------

select distinct a.ptid, b.age_index from ac_ehr_soliqua_hypo_lab_dx_bl a
